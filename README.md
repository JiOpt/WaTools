...
- 網站：<https://kawatool.com>（**v0.6.39**）
- 自訂網域：`kawatool.com`（GoDaddy 註冊，於 Firebase Hosting 綁定 DNS）
- Firebase 專案 ID：`watoolio`（Google 後台專案名；對外網域為 `kawatool.com`）
...
2. 輸入 `kawatool.com`（建議一併加入 `www.kawatool.com`）
3. 至 GoDaddy DNS 依 Firebase 指示設定 **TXT**（驗證）、**A**（根網域）、**CNAME**（`www` → `kawatool.com`）
4. 等待 Firebase 顯示 Connected 並完成 SSL 憑證

> Firebase 專案 ID 為 `watoolio`，與對外品牌網域 `kawatool.com` 不同，部署時請用 `--project watoolio`。
